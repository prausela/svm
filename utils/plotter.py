...
def add_roc_plot_from_positive_rates(positive_rates: list[dict[str, Any]], annotate: bool = False, font_size: int = 5,
    ...
    if annotate:
        for i, j in zip(FPRs, TPRs):
            plt.annotate('(%.2g, %.2g)' % (i, j), xy=(i, j), textcoords='offset points', xytext=(0, 7), ha='center',
                         fontsize=font_size, color=color)


def plot_roc_from_positive_rates(positive_rates: list[dict[str, Any]], title: str, save_file: str = None,
                                 annotate: bool = False, font_size: int = 5, label: str = None, color: str = "purple"):
    plt.figure()
...
def plot_multiple_roc_fromm_positive_rates(positive_rates_list: list[list[dict[str, Any]]], colors: list[str],
                                           title: str, save_file: str = None, annotate: bool = False,
                                           font_size: int = 5, labels: list[str] = None):
    plt.figure()
    __add_roc_plot_template__(title)
...
def plot_multiple_dicts(values_dict: dict[Any, dict[Any, Any]], title: str, xlabel: str, ylabel: str,
                        label_mapping: dict[Any, str] = None, save_file: str = None) -> None:
    plt.figure()
    plt.grid(True)
