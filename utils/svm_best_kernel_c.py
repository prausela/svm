...
def get_svm_confusion_matrix(cow_df: pd.DataFrame, grass_df: pd.DataFrame, sky_df: pd.DataFrame):
    ...
    for kernel in kernels:
        for c in c_values:
            ...
            plt.savefig(f"output/graphs/{kernel}/confusion_matrix_{c:.1f}.png")
            plt.close()
            ...
            print(f"Precision: {precision:.2f}, Error (Precision): {error_precision:.4f}")
            print(f"Accuracy: {accuracy:.2f}, Error (Accuracy): {error_accuracy:.4f}")
